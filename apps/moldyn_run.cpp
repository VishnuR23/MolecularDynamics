...
//      fluid and exits non-zero if any case falls outside the rounding
//      half-width of NIST's own published 5 significant figures. Does not
//      simulate.
//
...
// command line (reconstructed from argc/argv), the git revision the binary
// was built from (MOLDYN_GIT_SHA, regenerated by CMake on every build from
// `git describe --always --dirty`, so a build from a modified tree is
// stamped `<sha>-dirty` and cannot be mistaken for the clean commit), and a
// description of the host (OS and architecture only -- no username or
// hostname, since these files are committed to a public repository).
...
// Generated at build time by apps/GitSha.cmake; see apps/CMakeLists.txt.
...
const char* kUsage =
    ...
    "                       and exits; does not simulate. Each value is\n"
    "                       checked against the rounding half-width of\n"
    "                       NIST's own 5 published significant figures.\n"
...
// NIST publishes every value in the table above to exactly 5 significant
// figures, so the published number is the true one rounded to that
// precision: the true value lies within half a unit in the last printed
// digit. That half-width, per value, IS the precision of the reference --
// and therefore the tightest bound a comparison against it can honestly
// assert, and the loosest one worth asserting.
//
// The flat 5e-5 relative tolerance this replaced was up to 9.4x looser
// than that -- worst at W = -935.78, whose relative half-width is
// 0.005/935.78 = 5.34e-6 -- so a regression that broke the fifth
// published digit could still have passed. Every one of these values currently
// agrees to every digit NIST publishes; "within NIST's own rounding" is
// both the stricter test and the stronger claim.
//
// For the four LinearForceShift cases NIST's U_LRC is not a rounded
// measurement but an exact structural zero, so those are checked against
// a small absolute bound instead (kZeroLrcTolerance below).
double roundingHalfWidth(double referenceValue) {
    const double magnitude = std::abs(referenceValue);
    if (magnitude == 0.0) {
        return 0.0;
    }
    // Position of the 5th significant digit: for 4.3515e3 the last printed
    // digit sits at 10^(3-4) = 10^-1, so the half-width is 0.05.
    const double decadeExponent = std::floor(std::log10(magnitude));
    return 0.5 * std::pow(10.0, decadeExponent - 4.0);
}

constexpr double kZeroLrcTolerance = 1e-9;
...
int runNistTable(const std::string& dir) {
    ...
    std::cout << std::left << std::setw(4) << "cfg" << std::setw(5) << "trunc"
              ...
              << std::setw(14) << "u_ref" << std::setw(12) << "u_dev" << std::setw(12)
              << "u_tol" << std::setw(14) << "w_calc" << std::setw(14) << "w_ref"
              << std::setw(12) << "w_dev" << std::setw(12) << "w_tol" << std::setw(14)
              << "ulrc_calc" << std::setw(14) << "ulrc_ref" << std::setw(12) << "ulrc_dev"
              << std::setw(12) << "ulrc_tol" << "  status\n";
    ...
    for (const NistRef& r : kNistRefs) {
        ...
        // Deviations and tolerances are both expressed relatively, so the
        // ratio dev/tol is directly readable as "fraction of NIST's own
        // rounding half-width used".
        const double uDev = relativeDeviation(ev.energy, r.upair);
        const double wDev = relativeDeviation(ev.virial, r.wpair);
        const double uTol = roundingHalfWidth(r.upair) / std::abs(r.upair);
        const double wTol = roundingHalfWidth(r.wpair) / std::abs(r.wpair);

        // The LinearForceShift rows' U_LRC is an exact zero, not a rounded
        // measurement, so there is no last published digit to halve:
        // compare absolutely against kZeroLrcTolerance instead.
        const bool lrcIsZero = (r.ulrc == 0.0);
        const double lrcDev = lrcIsZero ? std::abs(lrc) : relativeDeviation(lrc, r.ulrc);
        const double lrcTolerance =
            lrcIsZero ? kZeroLrcTolerance : roundingHalfWidth(r.ulrc) / std::abs(r.ulrc);

        const bool pass = (uDev <= uTol) && (wDev <= wTol) && (lrcDev <= lrcTolerance);
        allPass = allPass && pass;
        worstRatio = std::max({worstRatio, uDev / uTol, wDev / wTol, lrcDev / lrcTolerance});
        ...
        std::cout << std::left << std::setw(4) << r.config << std::setw(5)
                  ...
                  << uDev << std::setw(12) << uTol << std::setw(14) << ev.virial
                  << std::setw(14) << r.wpair << std::setw(12) << wDev << std::setw(12) << wTol
                  << std::setw(14) << lrc << std::setw(14) << r.ulrc << std::setw(12) << lrcDev
                  << std::setw(12) << lrcTolerance << "  " << (pass ? "PASS" : "FAIL") << "\n";
    }

    std::cout << "\nworst case: " << worstRatio << " of the per-value bound\n";
    std::cout << (allPass
                       ? "ALL 12 CASES MATCH EVERY DIGIT NIST PUBLISHES (within each value's "
                         "5-significant-figure rounding half-width): PASS\n"
                       : "ONE OR MORE CASES FELL OUTSIDE NIST'S OWN 5-SIGNIFICANT-FIGURE "
                         "ROUNDING HALF-WIDTH: FAIL\n");
    return allPass ? 0 : 1;
}
...
public:
    StepSampler(moldyn::System& sys, const moldyn::LennardJones& lj, double& lastEnergy,
                double& lastVirial, std::size_t dof, std::string commandLine,
                std::ofstream* thermoFile, std::ofstream* trajFile, moldyn::Rdf* rdf,
                moldyn::Msd* msd, moldyn::Vacf* vacf)
        : sys_(sys),
          lj_(lj),
    ...
    // The number of thermalised degrees of freedom changes at the
    // equilibration/production boundary -- see runSimulation() for why --
    // and the reported temperature must change with it.
    void setDof(std::size_t dof) { dof_ = dof; }
    ...
    void record(long step, double t, const char* phase, bool sampleAnalysis) {
        ...
        if (trajFile_ != nullptr) {
            // XYZ has exactly one free-form comment line per frame, so the
            // full provenance every CSV carries in its '#' header goes
            // there: revision, host and the command line, same three facts
            // in the same words, just packed onto one line.
            const std::size_t n = sys_.size();
            (*trajFile_) << n << '\n';
            (*trajFile_) << "step=" << step << " time=" << t << " phase=" << phase
                         << " git_sha=" << gitSha() << " host=\"" << hostDescription()
...
// Degrees of freedom, which depend on what is driving the dynamics.
//
// Langevin gives every particle its own independent noise, so the
// centre-of-mass momentum random-walks and thermalises along with
// everything else: all 3N degrees of freedom carry kT/2 and dof = 3N.
// (Using 3N-3 here overestimates T by 3N/(3N-3) -- 0.9% at N=108.)
//
// Velocity-Verlet (NVE) and the Nose-Hoover chain both conserve total
// momentum: NVE exactly, the chain because it only ever rescales every
// velocity by one common factor. So if p_com is zero when production
// starts it stays zero, those 3 degrees of freedom are frozen, and
// dof = 3N-3. That "if" is the whole point of the re-zeroing below.
std::size_t dofLangevinDriven(const moldyn::System& sys) {
    return 3 * sys.size();
}

std::size_t dofMomentumConserving(const moldyn::System& sys) {
    return 3 * sys.size() - 3;
}
...
int runSimulation(const SimArgs& args, const std::string& commandLine) {
    ...
    const std::size_t dofLangevin = dofLangevinDriven(sys);
    const std::size_t dofNve = dofMomentumConserving(sys);
    ...
    if (args.thermoOut) {
        ...
        // The temperature column divides by the degrees of freedom that
        // are actually thermalised, and that differs by phase -- see
        // dofLangevinDriven/dofMomentumConserving in apps/moldyn_run.cpp.
        (*thermoFile) << "# temperature_dof: 3N=" << dofLangevin
                       << " while Langevin drives (equilibration, and Langevin production), "
                          "3N-3="
                       << dofNve
                       << " under the momentum-conserving integrators (NVE, Nose-Hoover), for "
                          "which the centre-of-mass momentum is re-zeroed at the "
                          "equilibration/production boundary\n";
    ...
    // Starts at 3N-3: fccLattice() has just zeroed p_com, so the 'start'
    // row really does have three frozen degrees of freedom. Switched to
    // 3N for Langevin equilibration immediately below.
    StepSampler sampler(sys, lj, lastEnergy, lastVirial, dofNve, commandLine,
                         thermoFile ? &(*thermoFile) : nullptr, trajFile ? &(*trajFile) : nullptr,
                         rdf ? &(*rdf) : nullptr, msd ? &(*msd) : nullptr,
                         vacf ? &(*vacf) : nullptr);
    ...
    // towards the target temperature before sampling begins. Langevin
    // thermalises the centre of mass too, so temperatures reported during
    // this phase use dof = 3N (see dofLangevinDriven above).
    sampler.setDof(dofLangevin);
    ...
    // The equilibration/production boundary. Langevin equilibration has
    // left the centre of mass with a thermal momentum of its own
    // (<v_com^2> = 3kT/N), and a drifting box contaminates every quantity
    // measured downstream: it adds v_com^2 * t^2 to the mean squared
    // displacement and a constant, non-decaying floor to the velocity
    // autocorrelation function, biasing both diffusion estimates upward by
    // different amounts. fccLattice() zeroes p_com once at construction,
    // which is not enough -- thousands of Langevin steps happen in
    // between. Re-zero it here, once, so the momentum-conserving
    // production integrators start (and therefore stay) at p_com = 0 and
    // dof = 3N-3 is actually true.
    //
    // Only here, and never inside a Langevin production loop: continually
    // projecting out the centre-of-mass motion a Langevin thermostat is
    // supposed to be generating would perturb the very dynamics it is
    // sampling. A Langevin production run therefore keeps dof = 3N, and
    // this one-off reset is a transient it relaxes out of within ~1/gamma.
